...
// table nodes luôn thẳng cột
const X_TABLE = 0
const X_MERGE = 440

// khoảng cách dọc giữa left / right của merge1
const FIRST_STACK_GAP = 60
...
export default function MergeDAGView({ dag }: Props) {
  ...
  if (dag.mode === "chain") {
    ...
    dag.edges.forEach((e, i) => {
      ...
      const isFirstMerge = stepIndex === 0
      const isLeft = i % 2 === 0

      let y =
        stepIndex * ROW_GAP +
        (stepIndex > 0 ? FIRST_STACK_GAP : 0)

      // merge1: left / right xếp dọc
      if (isFirstMerge && !isLeft) {
        y += FIRST_STACK_GAP
      }
      ...
      if (!seen.has(e.to)) {
        nodes.push({
          ...
          position: {
            x: X_MERGE,
            y: isFirstMerge ? y + FIRST_STACK_GAP : y,
          },
  ...
  /* ======================================================
     PAIRWISE MODE (KHÔI PHỤC ĐẦY ĐỦ)
     ====================================================== */
      ...
      edges.push(
        {
          ...
          type: "centered",
          data: {
            label: `${leftEdge.joinType}\n${leftEdge.leftKeys.join(",")} = ${leftEdge.rightKeys.join(",")}`,
          },
        },
        {
          id: `e-p-${i + 1}`,
          ...
          type: "centered",
          data: {
            label: `${rightEdge.joinType}\n${rightEdge.leftKeys.join(",")} = ${rightEdge.rightKeys.join(",")}`,
          },
        }
      )
